"""
Oncology Specialty Agent for CareFlow Orchestrator.

Analyses oncological indicators and produces structured oncology
...
class OncologyAgent(SpecialtyAgentBase):
    """Specialty Agent for oncology analysis.

    Analyses oncological indicators (e.g. tumour markers, staging, treatment
    options) and returns structured oncology recommendations.
    ...
    def analyze(self, case_summary: str, guidelines: List[str]) -> SpecialtyFindings:
        """Analyse oncological indicators for the given case.

        Args:
            case_summary: Plain-text summary of the patient case.
            guidelines: Oncology-specific clinical guidelines.

        Returns:
            :class:`~backend.schemas.SpecialtyFindings` with specialty
            ``"oncology"``, a summary string, and a list of action items.
        ...
        prompt = (
            "You are an expert oncologist reviewing a clinical case.\n\n"
            "## Case Summary\n"
            f"{case_summary}\n\n"
            "## Oncology Guidelines\n"
            f"{guidelines_text}\n\n"
            "## Instructions\n"
            "Analyse the case from an oncology perspective. Focus on tumour markers, "
            "staging, biopsy requirements, treatment options, and follow-up protocols.\n\n"
            "Respond with ONLY a valid JSON object in the following format (no markdown, no extra text):\n"
            "{\n"
            '  "specialty": "oncology",\n'
            '  "summary": "<concise oncology summary>",\n'
